<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST')
    {
        ...
        if($_POST['sure'] == 'Yes')
        {
            $q = "DELETE FROM COMMENTS WHERE comment_id = $id LIMIT 1;";  
            ...
            if(mysqli_affected_rows($dbc) == 1)
            {
                ...
                echo '
                <script>
                    location.replace("/list_comments_admin.php");
                </script>
            ';
            ...
            {
                echo '<p class="error">The comment could not be deleted due to error</p>';
                echo '<p>' . mysqli_error($dbc) . '<br>Query: ' . $q . '</p>';
            }
        }
        else
        {
            echo "<p>You chose not to delete the comment.</p>";
            echo '
                <script>
                    location.replace("/list_comments_admin.php");
                </script>
    ...
    else
    {
        ...
        else
        {
            ...
            printf("Are you sure you want to delete this coupon?");
